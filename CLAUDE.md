...
- `src/stores/common-store.ts` — settings panel/directions panel open state, costing settings, dateTime, map-ready flag. `Profile` enum and `profileEnum` zod schema live here.
  Costing settings are split into two scopes, each a `ScopedSettings` (`values` + an `enabled` map of param names): `shared` for the general options every profile understands, and `perProfile[profile]` for the options a profile owns privately. **Only enabled options are sent** — see "Costing settings" below.
...
`src/utils/build-costing-options.ts` turns the two scopes into one profile's payload: shared options first (filtered to those `generalSettings[profile]` lists, so pedestrian never receives `use_highways`), then the profile's own, which win where they overlap. `alternates` and `exclude_polygons` are request-level rather than costing options and are always sent; `use_geocoding` is client-only and never sent.

The advanced panel (`settings-panel.tsx`) renders one "Shared settings" section — the union of the selected profiles' `generalSettings`, minus any param a profile owns privately — plus one collapsible section per selected profile holding its `profileSettings`, edged in that profile's colour. **A Valhalla param belongs to exactly one of the two scopes**; a duplicate would be shadowed out of the shared section by the dedupe and effectively disappear (this happened to `maneuver_penalty`, listed both as "Turn Penalty" generally and "Maneuver Penalty" per profile). `build-costing-options.spec.ts` asserts the scopes stay disjoint for every profile. `settings-group-fields.tsx` renders a section; `setting-row.tsx` pairs each control with its checkbox and dims the row while it's unticked. Note the callback split: `onValueChange` writes the value, `onCommit` only re-routes — a commit must never write a value, or a slider drag gets reverted by a stale closure.
