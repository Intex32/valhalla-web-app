...
- **Segment metrics** (debugging view) — `src/utils/segment-metrics.ts` + `src/components/map/parts/route-segment-metrics.tsx`. Repaints **only the selected route** along a blue→red ramp, per road segment, by one of four normalised metrics (`cost/km`, `time/km`, `speed`, `cost/time`); clicking a segment opens `segment-info-popup.tsx` with the exact numbers. Off by default behind the switch in `src/components/directions/segment-metrics-control.tsx`; state lives in `directions-store.segmentMetric`.

  Granularity comes in two levels. `/route` carries `cost` per **maneuver** only, which draws immediately. `src/hooks/use-trace-attributes-query.ts` then walks the selected route's own shape back through `/trace_attributes` with `shape_match: 'edge_walk'` and the **same costing options** (they must match or the numbers drift), which splits the same path into graph **edges** — ~300 where the route had 10 maneuvers — and the view sharpens. Each edge's cost is the difference between consecutive `end_node.elapsed_cost` values; that field is undocumented but always emitted alongside `elapsed_time` under the `node.elapsed_time` filter key.

  **Intersections are their own thing.** Transition cost — the penalty for turning through a junction — is about a third of a typical route's total and has nothing to do with the length of the edge that follows, so folding it in made a 9 m link read as 3800 cost/km. Our Valhalla patches `trace_serializer.cc` to emit `transition_cost` beside `transition_time` (upstream serialises only the seconds, and the seconds bear almost no relation to the cost). `toTraceSegments()` subtracts it from the edge and returns it as a `TransitionNode` drawn as a dot on the junction, sized and coloured by cost, with its own popup. Verified: 992.7 edge + 468.0 transition = the route's 1460.7, no negative residuals. On an unpatched server the field is absent and the transition silently stays in the edge.

  **There is no OSM node id to show.** Valhalla keeps way ids on edges (`edge.way_id`) but drops OSM node ids when it builds the graph — neither `trace_attributes` nor `/locate` exposes one. The junction popup identifies the node by the pair of ways meeting there, its coordinate and an osm.org link instead.

  Valhalla reports a transition on the node that **ends the previous edge**, so everything shifts by one: `edges[i - 1].end_node` is the junction entering edge `i`, and the shape point they share is where the dot sits.

  `rampDomain()` spans the 5th–95th percentile of the segments' values and clamps the rest. The trim rounds **inward** so it always drops at least one segment per end — rounding to nearest trims nothing on a ten-maneuver route, which is the case that needs it (the 41 m arrival stub). It deliberately does **not** filter by length any more: that rule made sense while transitions inflated short edges, but after the split it dropped 226 of 314 segments, left the scale to the long motorway edges, and collapsed the speed ramp to 50–60 km/h with a third of the route clamped — colours stopped tracking the values. The junction dots are scaled against each other, never against the road segments — a turn penalty and a cost-per-kilometre are not the same quantity. Their invisible hit circles track the visible radius rather than sitting at a flat maximum: a route has ~100 junctions, and fat hit areas on the cheap ones swallow every click meant for the road between them.
